...
```RADIUS_PI_MAPPING_USER```| *(empty)* | Inject entries into the `[Mapping user]` section of `rlm_python.ini` at container start, mapping privacyIDEA `detail.user.<key>` fields to RADIUS reply attributes. Format: comma-separated `pi_key=Radius-Attribute` pairs. Example: `vpn_ip=Framed-IP-Address,group=Class`. Regenerated on every start (idempotent), so changing the value and restarting updates the mapping cleanly.
...
**Env-driven `[Mapping user]` for FreeRADIUS reply attributes**
- Added `RADIUS_PI_MAPPING_USER` env var (see [RADIUS parameters](#radius-parameters-freeradiusenv)). Takes a comma-separated list of `pi_key=Radius-Attribute` pairs and injects them into the `[Mapping user]` section of `rlm_python.ini` at container start, so privacyIDEA `detail.user.<pi_key>` values flow into the named RADIUS reply attribute on Access-Accept.
- Common use case: `RADIUS_PI_MAPPING_USER=vpn_ip=Framed-IP-Address` — the VPN Pooler's per-user allocated IP is returned to the NAS in the standard `Framed-IP-Address` attribute.
- `rlm_python3/entrypoint.sh` regenerates the block on every start between `# BEGIN auto-mapping-user` / `# END auto-mapping-user` markers (idempotent: changing the env var and restarting the container reliably updates the mapping without stacking duplicates).
- Image rebuild required to pick up the new entrypoint: `docker compose build freeradius && docker compose up -d --force-recreate freeradius`.
